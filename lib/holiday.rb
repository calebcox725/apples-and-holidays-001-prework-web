...
def add_supply_to_winter_holidays(holiday_hash, supply)
  ...
  holiday_hash[:winter].each {|holiday, supplies| supplies << "Balloons"}
end
...
def all_supplies_in_holidays(holiday_hash)
  ...
  holiday_hash.each do |season, holidays|
    puts "#{symbol_to_capitalized_string(season)}:"
    holidays.each do |holiday, supplies|
      puts "  #{symbol_to_capitalized_string(holiday)}: #{supplies.join(", ")}"
    end
  end
end
